...
**Purpose**: Stores a person's email addresses with support for multiple emails and one primary email.
...
├── is_primary (only one primary email per person)
├── is_verified, verified_at
└── Status tracking: status ('active', 'inactive', 'bounced')
```

#### PersonPhone

**Purpose**: Stores a person's phone numbers with support for multiple phones and one primary phone.
...
├── country_code
├── is_primary (only one primary phone per person)
├── is_verified, verified_at
└── Status tracking: status ('active', 'inactive', 'invalid')
```

#### PersonAddress

**Purpose**: Stores a person's addresses with support for multiple addresses and one primary address.

```
person_address
├── person_id → Person
├── Address fields: street_line1, street_line2, city, state_province
├── Geographic: postal_code, country_code, latitude, longitude
├── address_type ('home', 'work', 'mailing', 'billing')
├── is_primary (only one primary address per person)
└── Status tracking: status ('active', 'inactive', 'invalid')
```
...
- Accept fieldSelections: Record<string, string> (e.g., { "firstName": "primary", "lastName": "person-B-id" })
...
- mergeAllChannels: boolean — reassign all secondary channels to primary (isPrimary reset to false)
</Step>
<Step title="Channel and Relationship Updates">
- Reassign all person_channel records to primary person
- Update leads: SET person_id = primaryPersonId WHERE person_id IN (secondaryPersonIds)
- Update contacts: SET person_id = primaryPersonId WHERE person_id IN (secondaryPersonIds)
- Mark secondary persons as merged: SET merged_into_id = primaryPersonId, merged_at = NOW()
</Step>
</Steps>
...
**Purpose**: Links deals to contacts (many-to-many relationship).

```
DealContact
├── deal_id → Deal
├── contact_id → Contact
├── contact_role ('primary', 'decision_maker', 'influencer', 'end_user')
├── is_primary (only one primary contact per deal)
└── added_at, added_by
```
...
erDiagram
    ...
    CommissionPayment ||--o{ Deal : "commissions for"
    EntityTransfer ||--o{ Contact : "transfers for"
    EntityTransfer ||--o{ Lead : "transfers for"
    EntityTransfer ||--o{ Deal : "transfers for"
```
