...
Real Estate entities link to CRM based on business context:
...
- Global system stages are the exception: they have deterministic IDs based on `systemType` (for example `LSTG-NEW`, `LSTG-DISQUALIFIED`, `DSTG-CLOSED-WON`)
- Org-specific custom/override stages use the normal org-scoped sequence (`LSTG-ADNS-001`, `DSTG-ADNS-001`)
</Tab>
</Tabs>

<Note>
Existing IDs do not change when the organization is renamed. Organization user memberships also receive stable fallback avatar colors from the user sequence. The hue uses a golden-angle spread across a finite HSL palette; colors are display metadata and may repeat in very large organizations, while `public_id` remains the unique user-facing membership identifier.
</Note>
...
**Purpose**: Stores a person's email addresses with priority and verification status.

```
person_email
├── person_id → Person
├── email (unique across organization)
├── is_primary (only one primary per person)
├── is_verified, verified_at
├── email_type (personal, work, other)
├── bounce_status (none, soft_bounce, hard_bounce)
├── last_bounce_at, bounce_count
├── organization_id
...
**Purpose**: Stores a person's phone numbers with type classification and validation.

```
person_phone
├── person_id → Person
├── phone_number (standardized format)
├── is_primary (only one primary per person)
...
**Purpose**: Stores a person's physical addresses with geocoding and primary designation.
...
│   └── formatted_address (computed)
├── Geolocation: latitude, longitude, geocoded_at
├── Verification: is_verified, verified_at
├── organization_id
└── Timestamps: created_at, updated_at
```
