...
<Accordion title="PersonEmail, PersonPhone, PersonAddress">
**PersonEmail:**
```
person_email
├── person_id → Person
...
person_phone
├── person_id → Person  
├── phone_number (E.164 international format)
├── phone_type (mobile, home, work, fax)
├── is_primary (only one per person)
├── status (active, inactive, disconnected)
├── verification: is_verified, verified_at
├── sms_opt_in, call_opt_in
...
├── verification: is_verified, verified_at, verification_source
```
</Accordion>
...
- mergeAllChannels: boolean — reassign all secondary channels to primary
</Step>
...
- Mark secondary persons as merged_into primary person
</Step>
</Steps>
...
{
  "firstName": "primary",
  "lastName": "person-B-id", 
  "email": "person-C-id",
  "phoneNumber": "primary",
  "preferredContactMethod": "person-B-id"
}
```
</Accordion>
</AccordionGroup>
...
<Accordion title="DealContact (Multiple contact associations)">
**Purpose**: Enables deals to associate with multiple contacts beyond the primary relationship.

```
DealContact
├── deal_id → Deal
├── contact_id → Contact
├── role (decision_maker, influencer, user, technical_contact, etc.)
├── involvement_level (primary, secondary, minor)
├── decision_authority (high, medium, low, none)
├── communication_preference (email, phone, in_person)
├── status (active, inactive, opted_out)
├── added_by, added_at
├── notes (role-specific notes)
└── Unique constraint: (deal_id, contact_id)
```

**Key Rules:**
- Primary deal contact is still maintained via Deal.contact_id
- Additional contacts provide broader stakeholder visibility
- Role and involvement level inform communication strategies
- All associated contacts must belong to same organization
- Contact removal from deal triggers audit logging
</Accordion>
