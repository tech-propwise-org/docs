...
├── deal_id → Deal
├── contact_id → Contact
├── Role: role (primary, influencer, decision_maker, end_user)
├── Flags: is_primary_contact
└── Audit: organization_id, created_at
```

**Purpose**: Many-to-many linking table for complex deal structures with multiple stakeholders.
...
├── Status: is_primary, status (active, inactive)
├── Timing: start_date, end_date
└── Audit: organization_id, created_at, updated_at
```

**Purpose**: Associates contacts with companies while tracking roles and employment history.
