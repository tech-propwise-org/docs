...
<Note>
  **Key Rules**: Every Lead, Contact must link to a Person. Person preferences apply across all contexts (leads, deals, contacts). The `nationality` is stored on Person so leads and contacts share the same identity profile value. The `original_source` is set once when person first enters system. Languages array uses unified `UserLanguageEntry` format with code and proficiency per entry. `links` stores web and social profile links as JSONB entries with `type` and `url`; fixed `website`, `linkedin_url`, and `twitter_url` columns are no longer used. `person_address` rows use `is_primary` for the primary address and `address_type` for classification (`Home`, `Work`, etc.); there is no separate address label field.
</Note>
