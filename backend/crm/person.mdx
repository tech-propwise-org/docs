---
title: "CRM Module"
description: "Complete CRM module specification including Person, Lead, Contact, Deal entities with their relationships, architecture principles, and business rules."
---
...
<Info>
  Channel belongs to Person, not Lead (Person-centric architecture). Lead can reference `source_channel_id` for attribution (which channel it came through). One Person can have multiple channels of same type.
</Info>
...
The relationship is symmetric — if (A, B) is marked as not-duplicate, the system treats (B, A) equivalently. Each organization maintains its own override decisions.
...
1. Lead status must be 'qualified' or 'hot'
2. Person must have primary email and phone
3. Creates Contact with OneToOne relationship to Person
4. Marks Lead as converted with conversion timestamp
5. Transfers all stakeholder assignments to new Contact

### Deal creation requirements

1. Must be linked to existing Contact (not Lead directly)
2. Deal value must be positive
3. Expected close date cannot be in the past
4. Primary stakeholder assignment is required
