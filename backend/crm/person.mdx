...
<Note>
  **Key Rules**: Similar pattern to `person_email`, `person_phone`, `person_address`. Channel belongs to Person, not Lead (Person-centric architecture). Lead can reference `source_channel_id` for attribution (which channel it came through). The `channel_identity_id` links to detailed channel entities (WhatsAppUser, InstagramUser). One Person can have multiple channels of same type (e.g., multiple WhatsApp numbers).
</Note>
