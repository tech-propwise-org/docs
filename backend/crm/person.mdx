...
<Note>
  Every Lead, Contact must link to a Person. Person preferences apply across all contexts (leads, deals, contacts). The `original_source` is set once when person first enters system.
</Note>
...
Each person can have multiple addresses for different purposes. The system maintains one primary address per person for default communication and billing purposes. Coordinates are stored for location-based queries and mapping functionality.
</Tab>
</Tabs>
...
<Info>
  Channel belongs to Person, not Lead (Person-centric architecture). Lead can reference `source_channel_id` for attribution (which channel it came through). The `channel_identity_id` links to detailed channel entities (WhatsAppUser, InstagramUser). One Person can have multiple channels of same type.
</Info>
...
The relationship is symmetric — if (A, B) is marked as not-duplicate, the system treats (B, A) equivalently. Each organization maintains its own override decisions. Used by `PersonNotDuplicateService` to exclude pairs from duplicate detection.
