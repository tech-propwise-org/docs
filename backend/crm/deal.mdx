...
- Global system stages are the exception: they have deterministic IDs based on `systemType` (e.g., `LSTG-NEW`, `LSTG-DISQUALIFIED`, `DSTG-CLOSED-WON`)
...
Person contact information is stored in separate tables for flexibility:
...
person_phone
├── person_id → Person
├── phone_number (normalized format)
├── phone_type (mobile, home, work, fax, other)
├── country_code
├── is_primary
├── is_verified, verified_at
├── status (active, disconnected, do_not_call)
```
</Tab>
