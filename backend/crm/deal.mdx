...
<Tabs>
<Tab title="From lead">
Copy the explicitly selected LeadPropertyInterest criteria → DealPropertyInterest (1:1). If the lead interest has multiple InventoryUnit candidates, choose one unit for the deal.
...
<Tab title="Walk-in">
Create DealPropertyInterest with no originating interest and at least one criterion (an inventory unit selection is sufficient on its own).
