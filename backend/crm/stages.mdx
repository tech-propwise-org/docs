...
Stage history tracking enforces several important business rules:

<Tabs>
<Tab title="Data integrity">
- History records are immutable once created
- Duration calculations are automatically computed and stored
- Stage transitions must respect system type constraints
- Organization boundaries are strictly enforced
</Tab>
<Tab title="Audit requirements">
- All stage changes are tracked with user attribution
- Timestamps provide complete audit trail
- Stage change reasons are preserved in history notes
- Terminal stage behaviors are logged for compliance
</Tab>
<Tab title="System performance">
- History tables are indexed for analytics queries
- Bulk operations respect history recording
- Archive/restore operations maintain history integrity
- Export functions include complete stage timeline
</Tab>
</Tabs>
