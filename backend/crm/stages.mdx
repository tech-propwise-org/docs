...
The CRM uses a three-layer identity system:

1. **Person** - The hidden identity layer (single source of truth for personal details)
2. **Contact** - The business relationship layer (qualified customers) 
3. **Lead** - The sales opportunity layer (unqualified inquiries)
4. **Deal** - Links to Contact, not Person directly

### Entity relationships

```
Person (1) ←→ (*) Lead ←→ (1) Contact ←→ (*) Deal
     ↓
PersonEmail/PersonPhone/PersonAddress/PersonChannel
     ↓
Activity (polymorphic: entity_type/entity_id)
     ↓
Notes/Tags (polymorphic)
```
...
Single `entity_stakeholder` table handles assignment and commission across leads/deals:

```sql
entity_stakeholder
├── entity_type ('lead' | 'deal' | 'contact')
├── entity_id (Foreign Key to lead/deal/contact)
├── stakeholder_type ('user' | 'team')
├── stakeholder_id (user_id or team_id)
├── role ('assigned' | 'watcher' | 'collaborator')
├── commission_split_percentage (for deals)
...
- Individual payments created for each stakeholder based on split percentages
- Commission payments have states: `PENDING` → `APPROVED` → `PAID`

## Transfer system

Entity transfers allow reassignment between users/teams while preserving history:

```sql
entity_transfer
├── entity_type, entity_id (what's being transferred)
├── from_user_id, to_user_id (transfer direction)
...
- **Activity table**: Indexes timeline events across all channels
- **Channel tables**: Store full conversation data (whatsapp, instagram, etc.)
- **Polymorphic linking**: Activities link to leads/deals/contacts via entity_type/entity_id
...
- `property_viewing`, `site_visit` (real estate module)

## Notes system

Polymorphic notes system supports all CRM entities:
...
├── content (rich text)
├── note_type ('general' | 'follow_up' | 'meeting_notes')
...
Polymorphic task system for follow-ups and reminders:
...
├── entity_id (Foreign Key)
├── title, description
├── task_type ('follow_up' | 'call' | 'meeting' | 'email')
├── priority ('low' | 'medium' | 'high')
├── status ('pending' | 'completed' | 'cancelled')
├── due_date, completed_at
├── assigned_to_id → User (Foreign Key)
├── created_by, updated_by
```

## Event system

System-wide events for integration and automation:

```sql
event
├── event_type ('stage_changed' | 'entity_created' | 'assignment_changed')
├── entity_type, entity_id (source entity)
├── payload (JSON data)
├── triggered_by_id → User (Foreign Key)
...
Polymorphic tagging for categorization and filtering:

```sql
tag
├── name, color, description
├── tag_type ('lead_source' | 'interest' | 'priority' | 'custom')
...
```mermaid
erDiagram
    Person ||--o{ Lead : "has inquiries"
    Person ||--o{ Contact : "becomes customer"
    Contact ||--o{ Deal : "has opportunities"
    Contact ||--o{ ContactCompanyRole : "company associations"
    Company ||--o{ ContactCompanyRole : "contact associations"
    
    Lead }o--|| LeadStage : "current stage"
    Deal }o--|| DealStage : "current stage"
    
    Lead ||--o{ LeadStageHistory : "completed stages"
    Deal ||--o{ DealStageHistory : "completed stages"
    
    Lead ||--o{ EntityStakeholder : "assigned to"
    Deal ||--o{ EntityStakeholder : "assigned to"
    Contact ||--o{ EntityStakeholder : "assigned to"
    ...
    Person ||--o{ PersonEmail : "email addresses"
    Person ||--o{ PersonPhone : "phone numbers"
    Person ||--o{ PersonAddress : "addresses"
    Person ||--o{ PersonChannel : "channel connections"
    
    Lead ||--o{ Note : "notes"
    Deal ||--o{ Note : "notes"
    Contact ||--o{ Note : "notes"
    Person ||--o{ Note : "notes"
    
    Lead ||--o{ Task : "tasks"
    Deal ||--o{ Task : "tasks"
    Contact ||--o{ Task : "tasks"
    
    Lead ||--o{ EntityTag : "tags"
    Deal ||--o{ EntityTag : "tags"
    Contact ||--o{ EntityTag : "tags"
    Tag ||--o{ EntityTag : "entity associations"
```
