---
title: "Stages & history"
description: "Stage systems, history tracking, and analytics for leads and deals in the CRM module."
---

## Overview

The CRM stage system tracks leads and deals through configurable pipelines, maintaining comprehensive history for analytics and business insights. Both leads and deals use a two-tier architecture with global system stages and organization-specific customizations.

<Info>
The stage system is deeply integrated with the broader CRM architecture through entity relationships, assignment systems, activity correlation, and commission calculations.
</Info>

## Design principles

### History-only approach

History records track **completed stages only**. The current stage lives on the entity itself.

- `Lead.stage` / `Deal.stage` — current stage
- `Lead.stageEnteredAt` / `Deal.stageEnteredAt` — when the entity entered the current stage
- `lead_stage_history` / `deal_stage_history` — records of stages the entity has **left**

<Warning>
No history record is created when entering a stage. History is created only when **leaving** a stage. The current stage is never in history until you leave it.
</Warning>

### Two-tier architecture

Both lead and deal stages use a global-plus-override architecture:

1. **Global stages** (`organization = NULL`) — System-defined stages shared across all organizations, seeded on startup
2. **Organization-specific stages** (`organization = org_id`) — Custom stages created by the org, or overrides of global stages

**Lookup priority:** Organization-specific → Global. When resolving a stage by `systemType`, the service checks org-specific first, then falls back to global.

### Public ID system

All stages receive stable public identifiers using the shared CRM public ID allocator:

- Lead stages: `LSTG-{ORG_PREFIX}-{SEQUENCE}` (e.g., `LSTG-ADNS-001`)
- Deal stages: `DSTG-{ORG_PREFIX}-{SEQUENCE}` (e.g., `DSTG-ADNS-001`)
- Global system stages use deterministic IDs: `LSTG-NEW`, `LSTG-DISQUALIFIED`, `DSTG-CLOSED-WON`, etc.

<Note>
System stages have predictable public IDs based on their `systemType`, while custom stages use the organization-scoped sequence format.
</Note>

---

## Lead stage system

Lead stages use a `systemType` enum to define programmatic behavior. There are 5 system stages:

| systemType     | Default name | Public ID           | Behavior                                                 |
| -------------- | ------------ | ------------------- | -------------------------------------------------------- |
| `new`          | New          | `LSTG-NEW`          | Default stage when lead is created                       |
| `contacted`    | Contacted    | `LSTG-CONTACTED`    | Auto-transition when first activity is logged           |
| `qualified`    | Qualified    | `LSTG-QUALIFIED`    | Lead has been qualified                                  |
| `converted`    | Converted    | `LSTG-CONVERTED`    | Triggers conversion (sets `lead.convertedAt`)           |
| `disqualified` | Disqualified | `LSTG-DISQUALIFIED` | Triggers disqualification (sets `lead.disqualifiedAt`)  |

<Info>
System stages have `systemType` set; custom stages have `systemType = null`. The display name can be customized per organization, but `systemType` determines behavior. `isSystem`, `isConverted`, `isDisqualified`, and `isTerminal` are computed properties derived from `systemType`.
</Info>

```typescript
// Computed properties on LeadStage entity
get isSystem(): boolean { return this.systemType != null; }
get isConverted(): boolean { return this.systemType === SystemLeadStageType.CONVERTED; }
get isDisqualified(): boolean { return this.systemType === SystemLeadStageType.DISQUALIFIED; }
get isTerminal(): boolean { return this.isConverted || this.isDisqualified; }
get isNew(): boolean { return this.systemType === SystemLeadStageType.NEW; }
get isContacted(): boolean { return this.systemType === SystemLeadStageType.CONTACTED; }
```

### Lead terminal stage behaviors

<Tabs>
<Tab title="Conversion">
When a lead moves to the **Converted** stage (`isConverted = true`) and `convertedAt` is not already set:

1. Check if a Contact already exists for `lead.person` in this organization
2. If the Contact exists but is **archived**, auto-unarchive it (clear `isArchived`, `archivedAt`, `archivedBy`)
3. If no Contact exists, create one with `customerSince = NOW()` and `customerType = 'lead_conversion'`
4. Set `lead.convertedAt = NOW()`

<Note>
The idempotency check prevents duplicate contacts when a lead is moved to Converted multiple times or when the person already has a contact from a different lead. If the existing Contact is archived, the system auto-unarchives it rather than blocking conversion.
</Note>
</Tab>
<Tab title="Disqualification">
When a lead moves to the **Disqualified** stage (`isDisqualified = true`) and `disqualifiedAt` is not already set:

- Set `lead.disqualifiedAt = NOW()`
- If `stageChangeReason` is provided, store it in `lead.disqualificationNotes`

Setting `disqualificationReason` directly on the update DTO (without a stage change) also triggers `disqualifiedAt = NOW()` if it was previously null.

</Tab>
<Tab title="Auto-requalify">
When a lead moves **out of** a disqualified stage to any non-disqualified stage, and `disqualifiedAt` is set:

- Clear `lead.disqualifiedAt`
- Clear `lead.disqualificationReason`
- Clear `lead.disqualificationNotes`

This ensures that returning a lead to the pipeline automatically clears all disqualification data. No manual reset is needed.

</Tab>
</Tabs>

---

## Deal stage system

Deal stages also use a `systemType` enum. There are 5 system stages:

| systemType    | Default name | Public ID           | Behavior                                                      |
| ------------- | ------------ | ------------------- | ------------------------------------------------------------- |
| `new`         | New          | `DSTG-NEW`          | Default stage when deal is created                            |
| `proposal`    | Proposal     | `DSTG-PROPOSAL`     | Proposal sent                                                 |
| `negotiation` | Negotiation  | `DSTG-NEGOTIATION`  | In active negotiation                                         |
| `closed_won`  | Closed Won   | `DSTG-CLOSED-WON`   | Triggers deal closure (`deal.isWon = true`, `deal.closedAt`) |
| `closed_lost` | Closed Lost  | `DSTG-CLOSED-LOST`  | Triggers deal closure (`deal.isWon = false`, `deal.closedAt`) |

```typescript
// Computed properties on DealStage entity
get isSystem(): boolean { return this.systemType != null; }
get isClosedWon(): boolean { return this.systemType === SystemDealStageType.CLOSED_WON; }
get isClosedLost(): boolean { return this.systemType === SystemDealStageType.CLOSED_LOST; }
get isTerminal(): boolean { return this.isClosedWon || this.isClosedLost; }
get isNew(): boolean { return this.systemType === SystemDealStageType.NEW; }
get isProposal(): boolean { return this.systemType === SystemDealStageType.PROPOSAL; }
get isNegotiation(): boolean { return this.systemType === SystemDealStageType.NEGOTIATION; }
```

### Deal terminal stage behaviors

<Tabs>
<Tab title="Closing">
When a deal moves to a **terminal stage** (`isTerminal = true`):

1. Set `deal.isClosed = true`, `deal.isWon = newStage.isClosedWon`
2. Set `deal.closedAt = NOW()`, `deal.closedBy = currentUser`
3. If `isClosedWon`: calculate `deal.totalCommission = deal.value × (deal.commissionRate / 100)` and create `commission_payment` records for each deal stakeholder via the `entity_stakeholder` table

<Info>
The commission system creates individual payment records for each stakeholder assigned to the deal, with amounts calculated based on their configured commission split percentages.
</Info>

</Tab>
<Tab title="Reopening">
When a deal moves from a **terminal** stage back to a **non-terminal** stage:

1. Cancel all non-PAID commission payments (`PENDING` or `APPROVED` → `CANCELLED`)
2. If any payments are already `PAID`, throw an error — the deal cannot be reopened
3. Clear all closure fields: `isClosed = false`, `isWon = false`, `closedAt`, `closedBy`, `totalCommission`

<Warning>
Reopening a deal fully reverses the closure. If any commission payments have already been marked as PAID, the reopen is blocked because the financial transaction cannot be reversed from within the CRM.
</Warning>
</Tab>
</Tabs>

---

## Stage entities structure

### LeadStage entity

```typescript
LeadStage
├── id (Primary Key)
├── publicId (Org-scoped coded ID: LSTG-{ORG_PREFIX}-{SEQ} or LSTG-{SYSTEM_TYPE})
├── name (Display name, customizable per org)
├── color (Hex color code for UI display)
├── order (Integer, determines pipeline sequence)
├── systemType (SystemLeadStageType enum, null for custom stages)
├── organization_id (Foreign Key, NULL for global stages)
├── isActive (Boolean, for soft deletion)
├── created_at, updated_at, deleted_at
└── Computed Properties:
    ├── isSystem (derived from systemType)
    ├── isConverted, isDisqualified, isTerminal
    └── isNew, isContacted
```

### DealStage entity

```typescript
DealStage
├── id (Primary Key)
├── publicId (Org-scoped coded ID: DSTG-{ORG_PREFIX}-{SEQ} or DSTG-{SYSTEM_TYPE})
├── name (Display name, customizable per org)
├── color (Hex color code for UI display)
├── order (Integer, determines pipeline sequence)
├── systemType (SystemDealStageType enum, null for custom stages)
├── organization_id (Foreign Key, NULL for global stages)
├── isActive (Boolean, for soft deletion)
├── created_at, updated_at, deleted_at
└── Computed Properties:
    ├── isSystem (derived from systemType)
    ├── isClosedWon, isClosedLost, isTerminal
    └── isNew, isProposal, isNegotiation
```

---

## Stage history tracking

### History table structures

<Tabs>
<Tab title="lead_stage_history">
```sql
lead_stage_history (completed stages only)
├── id (Primary Key)
├── lead_id → Lead (Foreign Key, Indexed)
├── stage_id → lead_stage (Foreign Key, the stage that was completed)
├── entered_at (Timestamp, when lead entered this stage)
├── duration_seconds (Integer, calculated: created_at - entered_at)
├── next_stage_id → lead_stage (Optional FK, the stage moved to)
├── notes (Text, stage change reason/notes)
├── changed_by_id → User (Foreign Key, who initiated the change)
├── organization_id → Organization (Foreign Key, for data isolation)
├── created_at (Timestamp, also serves as "exited_at")
├── updated_at (Timestamp)
└── Indexes:
    ├── UNIQUE(lead_id, stage_id, entered_at) -- Prevent duplicate history
    ├── INDEX(organization_id, created_at) -- Analytics queries
    ├── INDEX(stage_id, created_at) -- Stage performance reports
    └── INDEX(changed_by_id) -- User activity tracking
```
</Tab>
<Tab title="deal_stage_history">
```sql
deal_stage_history (completed stages only)
├── id (Primary Key)
├── deal_id → Deal (Foreign Key, Indexed)
├── stage_id → deal_stage (Foreign Key, the stage that was completed)
├── entered_at (Timestamp, when deal entered this stage)
├── duration_seconds (Integer, calculated: created_at - entered_at)
├── next_stage_id → deal_stage (Optional FK, the stage moved to)
├── notes (Text, stage change reason/notes)
├── changed_by_id → User (Foreign Key, who initiated the change)
├── organization_id → Organization (Foreign Key, for data isolation)
├── created_at (Timestamp, also serves as "exited_at")
├── updated_at (Timestamp)
└── Indexes:
    ├── UNIQUE(deal_id, stage_id, entered_at) -- Prevent duplicate history
    ├── INDEX(organization_id, created_at) -- Analytics queries
    ├── INDEX(stage_id, created_at) -- Stage performance reports
    └── INDEX(changed_by_id) -- User activity tracking
```
</Tab>
</Tabs>

### Stage change notes

When stage transitions include a `stageChangeReason`, the system:
1. Stores the reason as `notes` in the stage history record
2. Creates a `note` record with `note_type = 'stage_change'`
3. For lead disqualification, also stores in `lead.disqualificationNotes`

---

## Stage analytics & metrics

### Pipeline velocity metrics

<Tabs>
<Tab title="Average time per stage">
```sql
-- Average time in each stage (lead pipeline)
SELECT
  ls.name as stage_name,
  ls.system_type,
  AVG(lsh.duration_seconds) as avg_seconds,
  AVG(lsh.duration_seconds) / 86400.0 as avg_days,
  PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY lsh.duration_seconds) as median_seconds,
  COUNT(*) as total_transitions,
  COUNT(DISTINCT lsh.lead_id) as unique_leads
FROM lead_stage_history lsh
JOIN lead_stage ls ON lsh.stage_id = ls.id
WHERE lsh.organization_id = :org_id
  AND lsh.created_at >= NOW() - INTERVAL '90 days'
GROUP BY ls.id, ls.name, ls.system_type
ORDER BY ls.order;
```
</Tab>
<Tab title="Conversion rates">
```sql
-- Conversion rate by stage with funnel analysis
SELECT
  ls.name as stage_name,
  ls.system_type,
  COUNT(*) as leads_reached,
  COUNT(CASE WHEN l.converted_at IS NOT NULL THEN 1 END) as converted,
  COUNT(CASE WHEN l.disqualified_at IS NOT NULL THEN 1 END) as disqualified,
  COUNT(CASE WHEN l.converted_at IS NOT NULL THEN 1 END)::float / NULLIF(COUNT(*), 0) as conversion_rate,
  COUNT(CASE WHEN l.disqualified_at IS NOT NULL THEN 1 END)::float / NULLIF(COUNT(*), 0) as disqualification_rate
FROM lead l
JOIN lead_stage ls ON l.stage_id = ls.id
WHERE l.organization_id = :org_id
  AND l.created_at >= NOW() - INTERVAL '90 days'
GROUP BY ls.id, ls.name, ls.system_type
ORDER BY ls.order;
```
</Tab>
<Tab title="Stage flow analysis">
```sql
-- Stage transition patterns and drop-off points
SELECT
  current_stage.name as from_stage,
  next_stage.name as to_stage,
  COUNT(*) as transition_count,
  AVG(lsh.duration_seconds) / 86400.0 as avg_days_in_stage,
  COUNT(*) * 100.0 / SUM(COUNT(*)) OVER (PARTITION BY current_stage.id) as transition_percentage
FROM lead_stage_history lsh
JOIN lead_stage current_stage ON lsh.stage_id = current_stage.id
LEFT JOIN lead_stage next_stage ON lsh.next_stage_id = next_stage.id
WHERE lsh.organization_id = :org_id
  AND lsh.created_at >= NOW() - INTERVAL '90 days'
GROUP BY current_stage.id, current_stage.name, next_stage.id, next_stage.name
ORDER BY current_stage.order, transition_count DESC;
```
</Tab>
<Tab title="Pipeline velocity">
```sql
-- Overall pipeline velocity and bottleneck identification
WITH pipeline_metrics AS (
  SELECT
    l.id as lead_id,
    l.created_at as lead_created,
    l.converted_at,
    l.disqualified_at,
    EXTRACT(EPOCH FROM (COALESCE(l.converted_at, l.disqualified_at, NOW()) - l.created_at)) / 86400.0 as total_pipeline_days,
    COUNT(lsh.id) as stages_completed
  FROM lead l
  LEFT JOIN lead_stage_history lsh ON l.id = lsh.lead_id
  WHERE l.organization_id = :org_id
    AND l.created_at >= NOW() - INTERVAL '90 days'
  GROUP BY l.id, l.created_at, l.converted_at, l.disqualified_at
)
SELECT
  AVG(total_pipeline_days) as avg_pipeline_days,
  PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY total_pipeline_days) as median_pipeline_days,
  PERCENTILE_CONT(0.9) WITHIN GROUP (ORDER BY total_pipeline_days) as p90_pipeline_days,
  AVG(stages_completed) as avg_stages_per_lead,
  COUNT(*) as total_leads,
  COUNT(CASE WHEN converted_at IS NOT NULL THEN 1 END) as converted_count,
  COUNT(CASE WHEN disqualified_at IS NOT NULL THEN 1 END) as disqualified_count
FROM pipeline_metrics;
```
</Tab>
</Tabs>

### Performance benchmarking

<Tabs>
<Tab title="Stage benchmarks">
```sql
-- Stage performance benchmarks by user/team
SELECT
  u.first_name || ' ' || u.last_name as user_name,
  ls.name as stage_name,
  COUNT(*) as transitions_handled,
  AVG(lsh.duration_seconds) / 86400.0 as avg_days_per_stage,
  COUNT(CASE WHEN next_ls.system_type = 'converted' THEN 1 END) as conversions,
  COUNT(CASE WHEN next_ls.system_type = 'disqualified' THEN 1 END) as disqualifications
FROM lead_stage_history lsh
JOIN users u ON lsh.changed_by_id = u.id
JOIN lead_stage ls ON lsh.stage_id = ls.id
LEFT JOIN lead_stage next_ls ON lsh.next_stage_id = next_ls.id
WHERE lsh.organization_id = :org_id
  AND lsh.created_at >= NOW() - INTERVAL '30 days'
GROUP BY u.id, u.first_name, u.last_name, ls.id, ls.name
ORDER BY u.last_name, ls.order;
```
</Tab>
<Tab title="Deal pipeline metrics">
```sql
-- Deal pipeline performance and revenue analytics
SELECT
  ds.name as stage_name,
  COUNT(*) as deals_in_stage,
  SUM(d.value) as total_value,
  AVG(d.value) as avg_deal_value,
  AVG(EXTRACT(EPOCH FROM (NOW() - d.stage_entered_at)) / 86400.0) as avg_days_in_current_stage,
  COUNT(CASE WHEN d.is_closed AND d.is_won THEN 1 END) as won_deals,
  SUM(CASE WHEN d.is_closed AND d.is_won THEN d.value ELSE 0 END) as won_value
FROM deal d
JOIN deal_stage ds ON d.stage_id = ds.id
WHERE d.organization_id = :org_id
  AND d.archived_at IS NULL
GROUP BY ds.id, ds.name, ds.order
ORDER BY ds.order;
```
</Tab>
</Tabs>

### Advanced analytics patterns

<AccordionGroup>
<Accordion title="Cohort analysis">
Track cohorts of leads/deals by creation period through their stage progression:

```sql
-- Monthly lead cohorts and their progression
WITH monthly_cohorts AS (
  SELECT 
    DATE_TRUNC('month', l.created_at) as cohort_month,
    l.id as lead_id,
    l.created_at,
    l.converted_at,
    l.disqualified_at
  FROM lead l
  WHERE l.organization_id = :org_id
    AND l.created_at >= NOW() - INTERVAL '12 months'
),
stage_progression AS (
  SELECT 
    mc.cohort_month,
    COUNT(*) as cohort_size,
    COUNT(CASE WHEN mc.converted_at IS NOT NULL THEN 1 END) as converted_count,
    AVG(EXTRACT(EPOCH FROM (COALESCE(mc.converted_at, NOW()) - mc.created_at)) / 86400.0) as avg_time_to_conversion
  FROM monthly_cohorts mc
  GROUP BY mc.cohort_month
)
SELECT * FROM stage_progression ORDER BY cohort_month;
```
</Accordion>

<Accordion title="Stage abandonment analysis">
Identify stages where leads/deals commonly exit the pipeline:

```sql
-- Stage abandonment patterns
SELECT 
  ls.name as exit_stage,
  COUNT(*) as exits,
  AVG(lsh.duration_seconds) / 86400.0 as avg_time_in_stage,
  COUNT(*) * 100.0 / SUM(COUNT(*)) OVER () as exit_percentage
FROM lead_stage_history lsh
JOIN lead_stage ls ON lsh.stage_id = ls.id
JOIN lead l ON lsh.lead_id = l.id
WHERE l.disqualified_at IS NOT NULL
  AND l.organization_id = :org_id
  AND lsh.created_at >= NOW() - INTERVAL '90 days'
GROUP BY ls.id, ls.name
ORDER BY exits DESC;
```
</Accordion>

<Accordion title="Activity correlation">
Correlate activity patterns with successful stage transitions:

```sql
-- Activity volume correlation with conversion success
SELECT 
  l.id,
  ls.name as final_stage,
  COUNT(a.id) as activity_count,
  COUNT(DISTINCT a.activity_type) as activity_types,
  l.converted_at IS NOT NULL as was_converted
FROM lead l
JOIN lead_stage ls ON l.stage_id = ls.id
LEFT JOIN activity a ON l.id = a.entity_id AND a.entity_type = 'lead'
WHERE l.organization_id = :org_id
  AND l.created_at >= NOW() - INTERVAL '90 days'
GROUP BY l.id, ls.name, l.converted_at
ORDER BY activity_count DESC;
```
</Accordion>
</AccordionGroup>

---

## Query patterns

The CRM system is optimized for common access patterns:

<Tabs>
<Tab title="Current pipeline state">
```sql
-- High-frequency operational queries (no history joins)
-- Optimized for dashboard and list views
SELECT 
  l.id,
  l.public_id,
  l.title,
  l.value,
  l.stage_entered_at,
  ls.name as stage_name,
  ls.color as stage_color,
  ls.system_type,
  p.first_name || ' ' || p.last_name as person_name,
  EXTRACT(EPOCH FROM (NOW() - l.stage_entered_at)) / 86400.0 as days_in_stage
FROM lead l
JOIN lead_stage ls ON l.stage_id = ls.id
JOIN person p ON l.person_id = p.id
WHERE l.organization_id = :org_id 
  AND l.archived_at IS NULL
ORDER BY l.stage_entered_at DESC;
```
</Tab>
<Tab title="Full stage timeline">
```sql
-- Analytical queries including history + current stage
-- Used for lead detail pages and timeline views
SELECT timeline.* FROM (
  -- Historical stages (completed)
  SELECT 
    lsh.stage_id,
    ls.name as stage_name,
    ls.public_id as stage_public_id,
    ls.system_type,
    lsh.entered_at,
    lsh.created_at as exited_at,
    lsh.duration_seconds,
    lsh.notes,
    u.first_name || ' ' || u.last_name as changed_by_name,
    'completed' as status
  FROM lead_stage_history lsh
  JOIN lead_stage ls ON lsh.stage_id = ls.id
  LEFT JOIN users u ON lsh.changed_by_id = u.id
  WHERE lsh.lead_id = :lead_id
  
  UNION ALL
  
  -- Current stage (active)
  SELECT 
    l.stage_id,
    ls.name as stage_name,
    ls.public_id as stage_public_id,
    ls.system_type,
    l.stage_entered_at as entered_at,
    NULL as exited_at,
    EXTRACT(EPOCH FROM (NOW() - l.stage_entered_at))::integer as duration_seconds,
    NULL as notes,
    NULL as changed_by_name,
    'current' as status
  FROM lead l
  JOIN lead_stage ls ON l.stage_id = ls.id
  WHERE l.id = :lead_id
) timeline
ORDER BY timeline.entered_at ASC;
```
</Tab>
<Tab title="Stage assignment queries">
```sql
-- Leads/deals by stage with stakeholder information
-- Used for assignment and workload management
SELECT 
  ls.public_id as stage_public_id,
  ls.name as stage_name,
  COUNT(*) as total_leads,
  COUNT(DISTINCT es.user_id) as assigned_users,
  string_agg(DISTINCT u.first_name || ' ' || u.last_name, ', ') as assigned_to
FROM lead l
JOIN lead_stage ls ON l.stage_id = ls.id
LEFT JOIN entity_stakeholder es ON l.id = es.entity_id AND es.entity_type = 'lead'
LEFT JOIN users u ON es.user_id = u.id
WHERE l.organization_id = :org_id
  AND l.archived_at IS NULL
  AND (es.role = 'assigned' OR es.role IS NULL)
GROUP BY ls.id, ls.public_id, ls.name, ls.order
ORDER BY ls.order;
```
</Tab>
<Tab title="Performance tracking">
```sql
-- Stage performance metrics with time-based filtering
SELECT 
  ls.name as stage_name,
  ls.public_id as stage_public_id,
  COUNT(DISTINCT lsh.lead_id) as leads_processed,
  AVG(lsh.duration_seconds) / 3600.0 as avg_hours_in_stage,
  PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY lsh.duration_seconds) / 3600.0 as median_hours,
  COUNT(CASE WHEN next_ls.is_converted THEN 1 END) as conversions,
  COUNT(CASE WHEN next_ls.is_disqualified THEN 1 END) as disqualifications
FROM lead_stage_history lsh
JOIN lead_stage ls ON lsh.stage_id = ls.id
LEFT JOIN lead_stage next_ls ON lsh.next_stage_id = next_ls.id
WHERE lsh.organization_id = :org_id
  AND lsh.entered_at >= :start_date
  AND lsh.created_at <= :end_date
GROUP BY ls.id, ls.name, ls.public_id, ls.order
ORDER BY ls.order;
```
</Tab>
</Tabs>

### Auto-stage transitions

<Tabs>
<Tab title="Auto-transition logic">
The activity system triggers automatic stage transitions based on business rules:

<Note>
When the first activity is logged for a lead in `NEW` stage, the system automatically moves it to `CONTACTED` stage and updates `lead.stageEnteredAt`. This applies to all activity types except automated system activities.
</Note>

**Activity types that trigger auto-transition:**
- Phone calls (inbound or outbound)
- Emails (manual sends, not automated campaigns)
- Meetings and appointments
- Site visits (for real estate leads)
- Manual notes with contact confirmation

**Activities that do NOT trigger auto-transition:**
- Automated email campaigns
- System-generated activities
- Bulk operations
- Integration sync activities
</Tab>
<Tab title="Implementation logic">
```typescript
// Auto-transition service logic
async triggerAutoStageTransition(lead: Lead, activity: Activity): Promise<void> {
  // Only auto-transition from NEW stage
  if (!lead.stage.isNew) return;
  
  // Skip auto-transitions for automated activities
  if (activity.isAutomated || activity.isSystemGenerated) return;
  
  // Skip if lead is already in terminal state
  if (lead.stage.isTerminal) return;
  
  // Find the CONTACTED stage for this organization
  const contactedStage = await this.stageService.findBySystemType(
    lead.organizationId,
    SystemLeadStageType.CONTACTED
  );
  
  if (!contactedStage) {
    throw new Error('CONTACTED stage not found for organization');
  }
  
  // Transition to CONTACTED stage
  await this.leadService.updateStage(lead.id, {
    stageId: contactedStage.id,
    notes: `Auto-transitioned due to ${activity.activityType} activity`,
    triggeredByActivity: activity.id
  });
}
```
</Tab>
<Tab title="Time-based transitions">
Organizations can configure time-based stage transitions for:
- Automatic nurturing sequence advancement
- Stale lead identification and reassignment
- Follow-up reminder escalation
- Opportunity aging alerts

```typescript
// Example configuration for time-based transitions
interface StageAutoTransition {
  fromStageId: string;
  toStageId: string;
  triggerAfterHours: number;
  conditions?: {
    noActivitySince?: number; // hours
    valueThreshold?: number;
    assignedOnly?: boolean;
  };
  actions?: {
    reassign?: boolean;
    notify?: string[];
    createTask?: boolean;
  };
}
```
</Tab>
</Tabs>

---

## Business rules

### Stage transition constraints

<AccordionGroup>
<Accordion title="Lead conversion rules">
- A lead cannot move to Converted if it has already been converted to a Contact (idempotency protection)
- Moving to Converted stage requires `lead.person` to be in a valid state with complete contact information
- Auto-Contact creation respects organization settings and user permissions
- Converted leads cannot be moved backward to active stages without clearing conversion data
- If a Contact exists but is archived, conversion automatically unarchives it rather than failing
- Conversion creates audit trail in both stage history and contact creation activity logs
</Accordion>

<Accordion title="Deal closure rules">
- Deal closure requires all mandatory fields to be populated (deal value, assigned stakeholders, property interests for real estate deals)
- Commission calculations must complete successfully before marking as Closed Won
- Terminal stage transitions trigger validation of all deal contacts and property interests
- Reopening validation checks commission payment status before allowing stage change
- Deal value must be positive for Closed Won outcomes
- All stakeholders must have valid commission split percentages totaling 100%
</Accordion>

<Accordion title="System stage enforcement">
- Custom stages cannot be inserted between system stages that have order dependencies
- Organizations cannot delete system stages even with overrides
- Stage reordering operations validate the entire pipeline before applying changes
- Bulk operations respect individual stage transition rules
- System stage behaviors cannot be disabled, only customized in name and appearance
- Auto-transitions (like new → contacted) cannot be manually bypassed
</Accordion>

<Accordion title="Commission calculation rules">
- Commission rates must be between 0 and 100 percent
- Deal value changes in terminal stages require commission recalculation
- Stakeholder changes in closed deals require manual commission adjustment approval
- Commission payment status determines whether deals can be reopened
- Real estate deals calculate commission based on property value and commission tier structure
- Commission splits among multiple stakeholders must total exactly 100%
</Accordion>

<Accordion title="Auto-transition triggers">
- New → Contacted: Triggered by first activity (call, email, meeting) logged against the lead
- Activity-based transitions cannot be manually bypassed but can be disabled per organization
- Auto-transitions respect stage change permissions and user roles
- Failed auto-transitions are logged but do not block the triggering activity
- Auto-transitions can be customized to skip certain activity types (e.g., automated emails)
</Accordion>
</AccordionGroup>

### Stage configuration rules

<AccordionGroup>
<Accordion title="Organization customization">
- Organizations can override global stage names and colors while preserving system behavior
- Custom stages can be inserted between system stages with proper order validation
- Stage visibility can be restricted by user role within an organization
- Pipeline templates can be applied to new organizations with customizable defaults
- Stage configuration changes require appropriate admin permissions and audit logging
</Accordion>

<Accordion title="Validation constraints">
- Stage order values must be unique within organization scope
- Terminal stages cannot be moved to non-terminal positions
- System stages cannot have their `systemType` changed or removed
- Color codes must be valid hex values for consistent UI rendering
- Stage names cannot exceed character limits or contain restricted characters
</Accordion>

<Accordion title="Pipeline integrity">
- Every organization must have all required system stages (cannot delete or fully hide)
- Pipeline gaps (missing order sequences) are automatically detected and flagged
- Orphaned entities with invalid stage references trigger data consistency alerts
- Stage deletion cascades require explicit confirmation and impact analysis
- Concurrent stage modifications are synchronized to prevent configuration conflicts
</Accordion>
</AccordionGroup>

### Data integrity enforcement

<AccordionGroup>
<Accordion title="Stage history consistency">
- Duration calculations are automatically computed and cannot be manually overridden
- History records are immutable once created (audit trail integrity)
- Concurrent stage changes are serialized to prevent race conditions
- History cleanup operations require admin privileges and full audit justification
- Orphaned history records are automatically flagged for review
</Accordion>

<Accordion title="Organization isolation">
- Stage configurations are strictly isolated by organization
- Global stage overrides do not affect other organizations
- Cross-organization stage references are prohibited
- Stage migration tools respect organization boundaries
- Deleted organizations trigger cascade cleanup of custom stages
</Accordion>

<Accordion title="Stage validation rules">
- Stage order values must be unique within organization scope
- System stages cannot be reordered to break logical progression (new → contacted → qualified)
- Terminal stages (converted/disqualified, closed_won/closed_lost) must have highest order values
- Custom stage names cannot duplicate system stage names within the same organization
- Stage color values must be valid hex codes for UI consistency
</Accordion>
</AccordionGroup>

---

## Stage history service layer

### Core service methods

<Tabs>
<Tab title="LeadStageService">
```typescript
class LeadStageService {
  async findBySystemType(orgId: string, systemType: SystemLeadStageType): Promise<LeadStage | null>
  async findByPublicId(publicId: string, orgId: string): Promise<LeadStage | null>
  async listForOrganization(orgId: string, includeInactive?: boolean): Promise<LeadStage[]>
  async createCustomStage(orgId: string, data: CreateStageDto): Promise<LeadStage>
  async updateStage(stageId: string, data: UpdateStageDto): Promise<LeadStage>
  async reorderStages(orgId: string, stageOrders: StageOrderDto[]): Promise<void>
  async deleteStage(stageId: string): Promise<void>
  async validateStageTransition(fromStage: LeadStage, toStage: LeadStage): Promise<void>
}
```
</Tab>
<Tab title="DealStageService">
```typescript
class DealStageService {
  async findBySystemType(orgId: string, systemType: SystemDealStageType): Promise<DealStage | null>
  async findByPublicId(publicId: string, orgId: string): Promise<DealStage | null>
  async listForOrganization(orgId: string, includeInactive?: boolean): Promise<DealStage[]>
  async createCustomStage(orgId: string, data: CreateStageDto): Promise<DealStage>
  async updateStage(stageId: string, data: UpdateStageDto): Promise<DealStage>
  async reorderStages(orgId: string, stageOrders: StageOrderDto[]): Promise<void>
  async deleteStage(stageId: string): Promise<void>
  async validateStageTransition(fromStage: DealStage, toStage: DealStage): Promise<void>
}
```
</Tab>
<Tab title="StageHistoryService">
```typescript
class StageHistoryService {
  async recordStageTransition<T extends Lead | Deal>(
    entity: T,
    fromStage: LeadStage | DealStage,
    toStage: LeadStage | DealStage,
    changedBy: User,
    notes?: string
  ): Promise<void>
  
  async getEntityTimeline<T extends Lead | Deal>(entityId: string): Promise<StageTimelineEntry[]>
  async getStageMetrics(orgId: string, dateRange: DateRange): Promise<StageMetrics>
  async getUserPerformance(userId: string, orgId: string, dateRange: DateRange): Promise<UserStagePerformance>
  async getPipelineVelocity(orgId: string, dateRange: DateRange): Promise<PipelineVelocityMetrics>
  async getConversionFunnelAnalysis(orgId: string, dateRange: DateRange): Promise<ConversionFunnelMetrics>
  async getStageAbandonmentAnalysis(orgId: string, dateRange: DateRange): Promise<AbandonmentAnalysis>
  async getCohortProgressionMetrics(orgId: string, cohortPeriod: CohortPeriod): Promise<CohortMetrics[]>
}
```
</Tab>
</Tabs>

### Stage history DTOs

<Tabs>
<Tab title="Timeline DTOs">
```typescript
interface StageTimelineEntry {
  stageId: string;
  stageName: string;
  stagePublicId: string;
  systemType?: SystemLeadStageType | SystemDealStageType;
  enteredAt: Date;
  exitedAt?: Date;
  durationSeconds?: number;
  notes?: string;
  changedByName?: string;
  status: 'completed' | 'current';
}

interface StageTransitionDto {
  entityId: string;
  entityType: 'lead' | 'deal';
  fromStageId: string;
  toStageId: string;
  notes?: string;
  changedBy: string;
  metadata?: {
    autoTransition?: boolean;
    triggeredByActivity?: string;
    bulkOperation?: string;
  };
}
```
</Tab>
<Tab title="Analytics DTOs">
```typescript
interface StageMetrics {
  stageId: string;
  stageName: string;
  entitiesProcessed: number;
  averageDurationDays: number;
  medianDurationDays: number;
  conversionRate?: number;
  abandonmentRate?: number;
  throughputRate: number;
}

interface PipelineVelocityMetrics {
  averagePipelineDays: number;
  medianPipelineDays: number;
  p90PipelineDays: number;
  averageStagesPerEntity: number;
  totalEntities: number;
  conversionRate: number;
  bottleneckStages: StageBottleneck[];
}

interface UserStagePerformance {
  userId: string;
  userName: string;
  transitionsHandled: number;
  averageStageTime: number;
  conversionRate: number;
  stageBreakdown: StagePerformanceMetric[];
}
```
</Tab>
<Tab title="Business rule DTOs">
```typescript
interface StageValidationResult {
  isValid: boolean;
  violations: StageRuleViolation[];
  warnings: StageRuleWarning[];
}

interface StageRuleViolation {
  rule: string;
  message: string;
  severity: 'error' | 'warning';
  field?: string;
}

interface BulkStageChangeResult {
  successful: number;
  failed: number;
  warnings: number;
  details: BulkChangeDetail[];
  estimatedCommissionImpact?: number;
}
```
</Tab>
</Tabs>

### Update rules

<Tabs>
<Tab title="System stage rules">
| Stage type | Name | Color | Order | Deletion |
| --- | --- | --- | --- | --- |
| Global (`org = NULL`) | Cannot modify | Cannot modify | Cannot modify | Cannot delete |
| System override (org stage with `systemType`) | Can customize | Can customize | Cannot change | Cannot delete |
| Custom stage (no `systemType`) | Can change | Can change | Via reorder API | Can delete if unused |

<Note>
Updating a global stage auto-creates an org-specific override instead of modifying the global record (lead stages). Deal stages throw an error suggesting the user create an override.
</Note>
</Tab>
<Tab title="Deletion constraints">
- **Global system stages** — Cannot be deleted (system integrity)
- **Organization system overrides** — Cannot be deleted (business logic dependency)
- **Custom stages with active leads/deals** — Cannot be deleted until entities are moved
- **Custom stages with historical data** — Require special admin approval for deletion
- **Custom stages with no usage** — Can be deleted freely by organization admins
</Tab>
</Tabs>

### Bulk operations

<Steps>
<Step title="Validation phase">
Validate each stage transition individually and collect all business rule violations before processing any changes.
</Step>
<Step title="Authorization check">
Verify user permissions for bulk operations and each specific stage transition based on organization role settings.
</Step>
<Step title="Impact analysis">
Calculate commission impacts, stakeholder notifications, and downstream system effects before committing changes.
</Step>
<Step title="Transaction execution">
Process changes in batches with proper transaction boundaries, logging progress and handling partial failures gracefully.
</Step>
<Step title="Post-processing">
Trigger events, update analytics, send notifications, and perform any required integration synchronization.
</Step>
</Steps>

### Stage reordering

<Steps>
<Step title="Validation phase">
Validate that the new order maintains system stage positioning requirements and doesn't break any business logic dependencies.
</Step>
<Step title="Transaction phase">
Update all affected stage order values within a single database transaction to ensure consistency.
</Step>
<Step title="Cache invalidation">
Clear stage ordering caches across all application instances to ensure immediate consistency.
</Step>
<Step title="Audit logging">
Record the reorder operation with before/after states for compliance and debugging.
</Step>
</Steps>

---

## Events & integration

### Stage change events

The system emits comprehensive events when entities change stages to enable integration with external systems:

<Tabs>
<Tab title="Lead stage events">
```typescript
// Event types for lead stage changes
interface LeadStageChanged {
  leadId: string;
  leadPublicId: string;
  leadTitle: string;
  personId: string;
  previousStageId: string;
  newStageId: string;
  previousStageType?: SystemLeadStageType;
  newStageType?: SystemLeadStageType;
  changedBy: string;
  changedAt: Date;
  reason?: string;
  duration?: number; // seconds in previous stage
  isConverted: boolean;
  isDisqualified: boolean;
  organizationId: string;
  metadata?: {
    autoTransition?: boolean;
    triggerActivity?: string;
    conversionDetails?: ContactCreationDetails;
  };
}
```
</Tab>
<Tab title="Deal stage events">
```typescript
// Event types for deal stage changes
interface DealStageChanged {
  dealId: string;
  dealPublicId: string;
  dealTitle: string;
  dealValue: number;
  contactId: string;
  previousStageId: string;
  newStageId: string;
  previousStageType?: SystemDealStageType;
  newStageType?: SystemDealStageType;
  changedBy: string;
  changedAt: Date;
  reason?: string;
  duration?: number; // seconds in previous stage
  isClosed: boolean;
  isWon?: boolean;
  commissionTotal?: number;
  organizationId: string;
  metadata?: {
    stakeholderCount: number;
    propertyInterests?: string[];
    reopenedFromClosed?: boolean;
  };
}
```
</Tab>
<Tab title="Commission events">
```typescript
// Event types for commission processing
interface CommissionPaymentCreated {
  dealId: string;
  dealPublicId: string;
  dealTitle: string;
  userId: string;
  userName: string;
  amount: number;
  commissionSplit: number;
  status: 'PENDING';
  dueDate: Date;
  organizationId: string;
  metadata: {
    dealValue: number;
    commissionRate: number;
    stakeholderRole: string;
  };
}

interface CommissionPaymentStatusChanged {
  paymentId: string;
  dealId: string;
  userId: string;
  oldStatus: CommissionPaymentStatus;
  newStatus: CommissionPaymentStatus;
  changedBy: string;
  changedAt: Date;
  amount: number;
  organizationId: string;
}
```
</Tab>
</Tabs>

### Event handling workflows

<AccordionGroup>
<Accordion title="Assignment system integration">
Stage changes automatically trigger stakeholder reassignment rules:

```typescript
interface StageAssignmentRule {
  fromStageType?: SystemLeadStageType | SystemDealStageType;
  toStageType: SystemLeadStageType | SystemDealStageType;
  assignmentAction: 'reassign' | 'escalate' | 'distribute' | 'retain';
  targetUsers?: string[];
  targetTeams?: string[];
  conditions?: {
    entityValue?: { min?: number; max?: number };
    activityCount?: { min?: number };
    timeInPrevious?: { min?: number }; // hours
  };
}
```

Examples:
- New → Contacted: Retain current assignment
- Qualified → Converted: Escalate to senior sales manager
- Proposal → Closed Won: Assign to customer success team
- Any → Disqualified: Assign to marketing for nurturing
</Accordion>

<Accordion title="Activity automation">
Stage transitions can trigger automated activity creation:

```typescript
interface StageActivityTrigger {
  stageId: string;
  systemType?: SystemLeadStageType | SystemDealStageType;
  activityType: ActivityType;
  delayHours?: number;
  template?: string;
  conditions?: {
    onlyIfNoRecentActivity?: boolean;
    minimumEntityValue?: number;
    requiredTags?: string[];
  };
}
```

Common patterns:
- Contacted → Create follow-up call task (24h delay)
- Qualified → Send qualification confirmation email
- Proposal → Schedule proposal review meeting
- Closed Won → Create onboarding task
</Accordion>

<Accordion title="External system sync">
Events are published to message queues for external system integration:

```typescript
// Message queue topics by event type
const EVENT_TOPICS = {
  'lead.stage.changed': 'crm-lead-stages',
  'deal.stage.changed': 'crm-deal-stages', 
  'commission.payment.created': 'crm-commissions',
  'stage.history.bulk.completed': 'crm-bulk-operations'
};

// External system webhook payloads
interface WebhookPayload {
  eventType: string;
  timestamp: Date;
  organizationId: string;
  entity: Lead | Deal;
  stageChange: StageTransitionDto;
  metadata: Record<string, any>;
}
```
</Accordion>
</AccordionGroup>

### Event subscribers

<AccordionGroup>
<Accordion title="Analytics tracking">
Stage change events automatically update analytics tables for real-time reporting:
- Pipeline velocity calculations
- Conversion rate tracking
- User performance metrics
- Stage duration analytics
</Accordion>

<Accordion title="External integrations">
Events are published to external systems for:
- CRM synchronization (Salesforce, HubSpot)
- Marketing automation triggers
- Commission payment processing
- Compliance and audit logging
- Real-time dashboard updates
</Accordion>

<Accordion title="Internal automation">
Stage changes trigger internal business logic:
- Task creation and assignment
- Email notification workflows
- Activity timeline updates
- Report generation scheduling
</Accordion>
</AccordionGroup>

### Integration with assignment system

<Tabs>
<Tab title="Assignment rules by stage">
The stage system is deeply integrated with the assignment system through `entity_stakeholder`:

```sql
-- Stakeholder assignments by stage (for workload distribution)
SELECT 
  ls.name as stage_name,
  u.first_name || ' ' || u.last_name as user_name,
  es.role as stakeholder_role,
  COUNT(*) as assigned_count,
  AVG(EXTRACT(EPOCH FROM (NOW() - l.stage_entered_at)) / 86400.0) as avg_days_in_stage
FROM lead l
JOIN lead_stage ls ON l.stage_id = ls.id
JOIN entity_stakeholder es ON l.id = es.entity_id AND es.entity_type = 'lead'
JOIN users u ON es.user_id = u.id
WHERE l.organization_id = :org_id
  AND l.archived_at IS NULL
GROUP BY ls.id, ls.name, u.id, u.first_name, u.last_name, es.role
ORDER BY ls.order, assigned_count DESC;
```

Organizations can configure automatic assignment based on stage transitions:
- New leads assigned to junior sales reps
- Qualified leads escalated to senior reps
- Closed deals assigned to customer success managers
- Disqualified leads assigned to marketing for nurturing
</Tab>
<Tab title="Activity correlation">
```sql
-- Activity patterns by stage for performance analysis
SELECT 
  ls.name as stage_name,
  a.activity_type,
  COUNT(*) as activity_count,
  AVG(EXTRACT(EPOCH FROM (a.created_at - l.stage_entered_at)) / 86400.0) as avg_days_after_stage_entry
FROM lead l
JOIN lead_stage ls ON l.stage_id = ls.id
JOIN activity a ON l.id = a.entity_id AND a.entity_type = 'lead'
WHERE l.organization_id = :org_id
  AND a.created_at >= l.stage_entered_at
  AND l.stage_entered_at >= NOW() - INTERVAL '90 days'
GROUP BY ls.id, ls.name, a.activity_type
ORDER BY ls.order, activity_count DESC;
```

Activity creation can automatically trigger stage transitions based on business rules configured per organization. The system tracks which activities led to successful conversions to optimize future automation.
</Tab>
<Tab title="Commission integration">
When deals reach Closed Won stage, the commission system:

1. Validates all stakeholders have commission splits totaling 100%
2. Calculates individual amounts based on deal value and commission rate
3. Creates `commission_payment` records with PENDING status
4. Links payments to the specific stage transition for audit trail
5. Triggers approval workflow for commission payments above threshold

```sql
-- Commission performance by deal stage progression
SELECT 
  dsh.stage_id,
  ds.name as stage_name,
  COUNT(DISTINCT dsh.deal_id) as deals_processed,
  COUNT(cp.id) as commission_payments_generated,
  SUM(cp.amount) as total_commission,
  AVG(d.value) as avg_deal_value
FROM deal_stage_history dsh
JOIN deal_stage ds ON dsh.stage_id = ds.id
JOIN deal d ON dsh.deal_id = d.id
LEFT JOIN commission_payment cp ON d.id = cp.deal_id
WHERE dsh.organization_id = :org_id
  AND dsh.created_at >= NOW() - INTERVAL '90 days'
GROUP BY dsh.stage_id, ds.name, ds.order
ORDER BY ds.order;
```
</Tab>
</Tabs>

---

## Data consistency guarantees

### Transaction boundaries

<AccordionGroup>
<Accordion title="Stage transition transactions">
All stage transition operations use ACID transactions ensuring:
- Stage history record creation with calculated duration
- Entity field updates (convertedAt, closedAt, stageEnteredAt)
- Commission payment generation (for closed won deals)
- Activity log entry creation
- Event publication to message queue
- Cache invalidation across application instances

**Rollback scenarios:** If any step fails, the entire transaction is rolled back and the entity remains in its previous stage until manual intervention.
</Accordion>

<Accordion title="Commission calculation transactions">
Commission payment creation uses distributed transactions across:
- Deal closure validation and field updates
- Stakeholder role and split percentage validation
- Individual commission payment record creation
- Audit trail and approval workflow initialization
- External payment system notification (where applicable)

**Consistency guarantee:** Commission totals always match deal value × commission rate, with proper stakeholder split allocation.
</Accordion>

<Accordion title="Bulk operation consistency">
Bulk stage changes (affecting multiple leads/deals) use:
- Batch processing with configurable chunk sizes
- Individual transaction per entity to prevent total rollback
- Progress tracking and error collection
- Partial success handling with detailed result reporting
- Automatic retry for transient failures
</Accordion>
</AccordionGroup>

### Concurrent access handling

<AccordionGroup>
<Accordion title="Optimistic locking">
Stage changes use entity versioning to prevent conflicting updates:
- Version field updated on every entity modification
- Stage change operations validate current version before proceeding
- Concurrent modifications result in clear error messages requiring refresh
- Client applications handle version conflicts with user-friendly retry mechanisms
</Accordion>

<Accordion title="Stage configuration locking">
Administrative stage configuration changes use exclusive locks:
- Stage reordering operations lock the entire stage configuration
- Custom stage creation/deletion prevents concurrent pipeline modifications
- System stage override operations are serialized per organization
- Configuration changes trigger organization-wide cache invalidation
</Accordion>

<Accordion title="History integrity">
Stage history records maintain strict consistency:
- History insertion uses SELECT FOR UPDATE on entity table
- Duration calculations are atomic with history creation
- Concurrent stage changes are serialized per entity
- Failed transitions leave no partial history records
</Accordion>
</AccordionGroup>

---

<CardGroup cols={2}>
  <Card title="Lead" icon="bullseye" href="/backend/crm/lead">
    Lead entity and pipeline behavior.
  </Card>
  <Card title="Deal" icon="handshake" href="/backend/crm/deal">
    Deal entity, closure, and commission generation.
  </Card>
  <Card title="Entity views" icon="columns-3" href="/backend/entity-views">
    Kanban and list view APIs with stage grouping.
  </Card>
  <Card title="Enums" icon="list" href="/backend/crm/enums">
    Stage system type enum values.
  </Card>
</CardGroup>