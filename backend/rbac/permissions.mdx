...
### Advanced queries for permission resolution

<CodeGroup>
```typescript Effective Permissions Query
const effectivePermissions = await em.createQueryBuilder(Permission, 'p')
  .distinct()
  .leftJoin('p.roles', 'r')
  .leftJoin('r.userOrgRoles', 'uor')
  .leftJoin('r.teamMembershipRoles', 'tmr')
  .leftJoin('tmr.teamMembership', 'tm')
  .where(`
    (uor.user = ? AND uor.organization = ? AND uor.isDeleted = false) OR
    (tm.user = ? AND tm.team.organization = ? AND tm.isDeleted = false AND tmr.isDeleted = false)
  `, [userId, orgId, userId, orgId])
  .andWhere('p.isDeleted = false AND r.isDeleted = false')
  .getResult();
```

```typescript Team Available Permissions
const teamAvailablePermissions = await em.createQueryBuilder(Permission, 'p')
  .leftJoin('team_available_permissions', 'tap', 'tap.permission_id = p.id')
  .where('tap.team_id = ? AND p.scope = ?', [teamId, PermissionScope.ORG])
  .orWhere('p.scope = ?', [PermissionScope.TEAM])
  .andWhere('p.isDeleted = false')
  .getResult();
```

```typescript Role Hierarchy Query
const roleHierarchy = await em.createQueryBuilder(Role, 'r')
  .leftJoinAndSelect('r.permissions', 'p')
  .where('r.organization = ? OR r.isSystem = true', [orgId])
...
<Tabs>
<Tab title="ORG Permissions">
- **Assignable to**: Both ORG and TEAM roles
- **Scope**: Organization-wide access
- **Team Limitation**: Must be in team's `availablePermissions` when assigned to team roles
- **Inheritance**: Team roles inherit from organization context
</Tab>
<Tab title="TEAM Permissions">
- **Assignable to**: Only TEAM roles
- **Scope**: Team-specific access
- **Limitation**: Cannot be assigned to ORG roles
- **Context**: Always evaluated within team membership
</Tab>
<Tab title="System Permissions">
- **Special Rules**: `isSystemSensitive` permissions only for Owner
- **Global Scope**: Applied across all organizations
- **Admin Access**: `hasAdminAccess` permissions require elevated roles
- **Write Access**: `hasWriteAccess` enables modification operations
</Tab>
</Tabs>
