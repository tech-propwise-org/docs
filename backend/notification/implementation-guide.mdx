...
**Request Body:** `{ muteAll?: boolean, channelPreferences?: Partial<Record<Channel, boolean>>, typePreferences?: Partial<Record<NotificationType, boolean>> }`
...
- `muteAll = true` blocks all notifications except critical system alerts.
...
- **Email Service:** Uses the shared `EmailService`. Templates are managed in the email service, not the notification module.
- **WebSocket Gateway:** Real-time delivery via `NotificationGateway`. Tenant-scoped notifications use `user:{userId}` rooms; tenantless use `identity:{userId}` rooms.
- **User Module:** Validates recipient existence and org membership. Retrieves emails and display names.
- **Organization Module:** Enforces tenant isolation. Provides org context for preference defaults.
