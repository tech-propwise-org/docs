...
This guide explains how to add new notification types to the PropWise notification system. Use it when implementing notifications triggered by CRM, Real Estate, Access, or System events.
...
<Warning>
**Auth-entry exception:** public/identity invitation acceptance must not wait on courtesy notifications after the invitation has been committed. `InvitationService.acceptInvitation()` and `acceptInvitationForExistingUser()` emit `INVITATION_ACCEPTED` post-commit and non-blocking, because the invitee's login/org-selection path is more important than notifying the inviter. The listener still logs failures, but notification timeouts must never delay or roll back successful invitation acceptance.
</Warning>

<Note>
Some handlers like `EventInviteNotificationHandler` only mark notifications as actioned — the actual business logic (RSVP update) happens via separate API calls using payload IDs, following the tenantless invitation pattern.
</Note>

### AI Agent `notify_team_member` action (assignees mode)

When an AI agent action is configured with `notifyAssignees: true`, the action notifies every user with an active assignment on the triggering conversation (resolved at run time) in addition to any users mentioned in the action's `instructions`. This mode is exposed without requiring a mention in the instructions, allowing agents to notify "the assignees" dynamically. The `AiAgentContextBuilderService` renders an additional "Recipients: every user assigned to this conversation, plus anyone named in these instructions" note in the action guidelines block. The `notifyAssignees` flag is persisted in the action config and validated as a boolean in `ActionConfigDto`.

### Transactional persistence

**Transactional persistence:** `NotificationService.createNotificationInTransaction(em, dto)` writes using the caller's transactional `EntityManager` (see reminder firing and other nested-org flows). The caller must invoke `enqueuePreparedDeliveries(deliveries)` after the surrounding transaction commits when the result is non-null, `isDuplicate` is false, and `deliveries` is non-empty. The same self-notification guard as `createNotification()` applies: when `senderId === recipientId` and `includeSender` is false, the method returns `null` immediately without resolving the notification type in the registry.

**Bulk transactional persistence:** `createBulkNotificationInTransaction(em, dto, skipInApp)` is the multi-recipient counterpart. It persists the shared event + recipient rows and **prepares** the deliveries on the caller's `EntityManager`, returning `{ event, recipients, deliveries }` — it does **not** enqueue. The caller must call `enqueuePreparedDeliveries(deliveries)` **after** the surrounding transaction commits (enqueuing inside a transaction risks orphan queue jobs if it later rolls back). The public `createBulkNotification(dto)` is a thin wrapper: it runs `createBulkNotificationInTransaction` inside its own `executeInOrg` and then enqueues after commit, so every listener caller keeps after-commit semantics. The one accepted exception is the workflow `notify_user` step, which calls `createBulkNotificationInTransaction(context.em, …)` on the open step transaction and enqueues within it — safe only because `WORKFLOW_STEP_NOTIFICATION` delivers `IN_APP` only (no external I/O, so no orphan-job risk worth threading an after-commit hook through the engine's step-transaction boundary). Calling the forking `createBulkNotification` from inside the open step transaction would instead deadlock on the shared `organization` FK lock.
...
Core entity representing a notification. One event can be shared across multiple recipients (via `createBulkNotification`).
...
interface NotificationEvent {
  ...
  updatedAt: Date;
  triggeredBy: User;
  organization?: Organization; // NULL for tenantless notifications
...
interface NotificationRecipient {
  ...
  status: NotificationStatus; // PENDING, READ, DISMISSED, EXPIRED, ACTIONED
  readAt?: Date;
  dismissedAt?: Date;
  actionResults: ActionResult[];
  isDeleted: boolean; // soft-delete flag (default filter excludes these)
  createdAt: Date;
  updatedAt: Date;
...
Tracks delivery attempts per channel for each recipient.
...
Retrieve paginated notifications for the current user in the current organization.

**Query Parameters:** `page?: number` (default 1), `limit?: number` (default 20), `status?: NotificationStatus[]`, `category?: NotificationCategory[]`, `priority?: NotificationPriority[]`

**Response:** `{ items: NotificationDto[], meta: PaginationMeta }`

### PATCH /notifications/:id

Update notification status (mark as read/dismissed).

**Request Body:** `{ status: NotificationStatus }`

### POST /notifications/:id/actions/:actionName

Execute an action on a notification. Action handlers process the business logic.

**Request Body:** `{ data?: Record<string, any> }`

**Response:** `{ success: boolean, message?: string, redirectUrl?: string }`

### GET /notification-preferences

Get current user's notification preferences for the current organization.

### PUT /notification-preferences

Update notification preferences.

**Request Body:** `{ categorySettings?, subcategorySettings?, typeSettings? }` — sparse tri-state maps (see § Preference model). Global `muteAll` was removed; mute cascade ends at category level.

### Tenantless endpoints

All tenantless endpoints use `IdentityTokenGuard` instead of `AuthGuard`. The controller also has `@IdentityTokenOnly()` so the global `SessionActivityInterceptor` skips its refresh-token cookie check (tenantless requests don't have org context and may not carry the cookie on cross-site deployments):

- **GET /notifications/tenantless** — List tenantless notifications (no org context)
- **GET /notifications/tenantless/unread-count** — Unread tenantless notification count
- **PUT /notifications/tenantless/mark-all-read** — Mark all tenantless as read
- **PUT /notifications/tenantless/:id/mark-read** — Mark one as read
- **POST /notifications/tenantless/:id/action** — Execute tenantless action
- **DELETE /notifications/tenantless/:id** — Delete a tenantless notification

## Business rules & delivery

### Delivery rules

- In-app notifications are always attempted first.
- Email delivery respects user preferences and organization settings.
- Failed deliveries are retried up to 5 times with exponential backoff.
- Deliveries stop retrying after 24 hours.

### User preferences

- Channel preferences override type preferences — if email is disabled globally, no email notifications are sent regardless of per-type settings.
- New users get default preferences based on organization settings.
- Preferences are per-organization (users can have different settings per org).
- Tenantless notifications bypass preference checks entirely.
...
- **Email Service:** Uses the shared `EmailService`.
...
| Is there an **existing category** (CRM, REAL_ESTATE, ACCESS, TASK, DOCUMENT, SYSTEM)? | Yes                                       | Use it in `NotificationTypeConfig.category`.                                                                                                                                     |
...
|                                                                                     | RBAC (Access, Invitation, Role, Permission) | Use `listeners/rbac-event.listener.ts`.                                                                                                                                          |
  ...
  <Step title="Register config in NotificationTypeRegistry">
    **File:** `src/modules/notification/notification-type.registry.ts`
    ...
    this.register({
      ...
      category: NotificationCategory.CRM, // or REAL_ESTATE, ACCESS, TASK, DOCUMENT, SYSTEM
