...
description: "Detailed technical specification for the AI Conversation System including decision pipeline, LLM integration architecture, and response generation"
---
...
graph TD
    ...
    H --> I{Mode?}
    I -->|OFF| J[Skip]
    I -->|AUTO_REPLY| K[Generate + send]
    I -->|SUGGEST_ONLY| L[Generate + emit WebSocket]
    I -->|DRAFT| M[Generate + emit WebSocket with draft]
    K --> N[Check escalation triggers]
    L --> N
    M --> N
    N --> O[Compose prompt context]
...
<Tabs>
  ...
  <Tab title="Google Gemini">
    ...
    - **Features:** Multimodal support, fast inference
  </Tab>
  <Tab title="Anthropic">
...
| Average AI messages per conversation | `AVG(aiMessageCount)` where `> 0` | Per org, per period |
| Response time by mode | Message timestamps | Per mode, percentiles |
| Token usage | LLM provider responses | Per org, per model |
| Cost tracking | Token usage × provider pricing | Per org, per period |

<Check>
These metrics enable organizations to monitor AI performance, cost optimization, and customer satisfaction.
</Check>