...
Inbound message enters the webhook processing pipeline.
</Step>

<Step title="Phase A: Persist message">
Message is saved and conversation is updated (transactional).
</Step>
...
<AccordionGroup>
<Accordion title="Check conversation.aiMode">
- `OFF` → skip
...
<Accordion title="Check escalation triggers">
If escalation triggered → set aiMode = OFF, notify agent, skip
</Accordion>

<Accordion title="Process response">
...
<Warning>
All AI errors are logged with full context but do not throw. The agent is never blocked by AI failures.
</Warning>
...
<Check>
These metrics provide visibility into AI adoption, effectiveness, and escalation patterns to help optimize the system over time.
</Check>