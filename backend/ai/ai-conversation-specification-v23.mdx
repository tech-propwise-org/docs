---
title: "AI Conversation Specification v23"
description: "Complete specification for the AI Conversation System with automated responses, suggestion modes, and LLM provider integration"
---
...
Inbound Message Received and enters Phase A for persistence
</Step>
...
Non-critical, isolated try-catch operations including:
...
<Step title="AI Response Processing">
...
<Card title="Anthropic" icon="claude">
...
Handle response based on AI mode configuration
</Step>
...
<Warning>
All AI errors are non-blocking. Agents are never prevented from working due to AI failures.
</Warning>
...
| Average response time             | Time from inbound message to AI response sent         | Per org, per period |
| Token usage                       | Sum of `tokensUsed` from LLM responses                | Per org, per period |
| Provider performance              | Success rate, avg latency by provider                 | Per org, per period |

<Tip>
These metrics can be exposed via the existing analytics dashboard or a dedicated AI insights panel.
</Tip>

### Dashboard Implementation

Analytics will be integrated into the existing organization dashboard with:

- Real-time AI conversation count
...
<Note>
Detailed analytics implementation will be covered in a separate specification document.
</Note>