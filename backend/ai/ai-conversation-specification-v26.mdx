---
title: "AI Conversation System Specification v26"
description: "Comprehensive specification for the AI conversation system enabling automated and AI-assisted responses within the unified messaging module"
---

# AI Conversation System Specification v26
...
Non-critical, isolated try-catch processing including:
...
2. **Knowledge Context** — relevant chunks from the RAG pipeline via `EmbeddingService.generateEmbedding(query, apiKey)` + pgvector cosine search
...
Handle response based on conversation AI mode (AUTO_REPLY, SUGGEST_ONLY, or DRAFT)
</Step>
...
When `findAiPromptTemplate(...)` finds no matching `AI_PROMPT` `MessageTemplate` for the conversation, the conversation pipeline falls back to a hardcoded string.

<Note>
The `SystemPrompt` entity that previously stored an org-overridable default has been removed. The fallback is now a hardcoded constant in the service.
</Note>
