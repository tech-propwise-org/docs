---
title: "AI Conversation System Specification v32"
description: "Comprehensive specification for the AI conversation system, including customer-facing messaging AI, conversation modes, LLM integration, and automated response generation"
---

# AI Conversation System Specification v32
...
graph TD
    ...
    M -->|Escalation triggered| N[Set aiMode = OFF, notify agent, skip]
...
<Note>
**Future Enhancement:** For high-volume deployments, AI processing can be moved to a dedicated pg-boss queue (`ai-response`) to decouple it from the webhook worker entirely. The current Phase B approach is simpler and sufficient for initial rollout.
</Note>
...
<Warning>
When `findAiPromptTemplate(...)` finds no matching `AI_PROMPT` `MessageTemplate` for the conversation, the conversation pipeline falls back to a hardcoded string. It is **not** a DB row — the `SystemPrompt` entity that previously stored an org-overridable default has been removed.
</Warning>
...
<Check>
This specification provides a comprehensive framework for AI-powered conversation handling while maintaining clear boundaries between customer-facing messaging AI and internal CRM assistance.
</Check>