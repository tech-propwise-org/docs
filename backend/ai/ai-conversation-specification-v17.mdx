---
title: "AI Conversation Specification v17"
description: "Comprehensive specification for the AI conversation system with four interaction modes, LLM provider abstraction, and webhook integration"
---
...
The system supports four distinct modes of AI interaction, each providing different levels of automation and human oversight:
...
<Warning>
The system has strict performance requirements for real-time messaging:
</Warning>
...
- **Timeout handling:** If LLM call exceeds 8s, abort and log warning. Do not retry in the message pipeline.
...
The system uses a unified interface to support multiple LLM providers:
...
| ------------- | ----------------------- | --------------------- | --------------------- |
| OpenAI        | `openai` npm package    | GPT-4o, GPT-4o-mini   | Primary recommended   |
| Google Gemini | `@google/generative-ai` | Gemini 2.0 Flash, Pro | Cost-effective option |
| Anthropic     | `@anthropic-ai/sdk`     | Claude Sonnet, Haiku  | High-quality responses|
...
<Tip>
The system automatically manages token limits to prevent API errors and control costs.
</Tip>
...
<Warning>
All AI processing errors are non-blocking to ensure message delivery reliability.
</Warning>
...
| Response time | Time from message received to AI response sent | Average, P95 |
| Token usage | Sum of `tokensUsed` from LLM responses | Per org, per period |
| Cost tracking | Token usage × provider pricing | Per org, per period |

<Note>
Analytics data should be aggregated daily and stored in a separate analytics table for efficient reporting and dashboard queries.
</Note>