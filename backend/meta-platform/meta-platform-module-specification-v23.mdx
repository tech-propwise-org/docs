---
title: "Meta Platform Module Specification v23"
description: "Complete specification for the Meta Platform integration module including Graph API service, OAuth flows, webhook validation, and error handling for Facebook and Instagram messaging."
---
...
Creates a Meta Resumable Upload session under the configured app id and uploads raw bytes, returning the asset handle (`h`) used for WhatsApp template media header examples
</Accordion>

<Accordion title="delete<T>">
**Signature:** `(path, token, version?, circuitKey?, baseUrl?)`

DELETE request (no body or query params — append query params to `path` if needed). Used by messaging account cleanup for webhook unsubscription and app-permission revocation.
</Accordion>
</AccordionGroup>
...
<Check>
The WebhookEventLog entity ensures reliable webhook processing with built-in idempotency and error tracking.
</Check>