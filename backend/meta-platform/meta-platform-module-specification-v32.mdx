...
description: "Comprehensive specification for Meta Platform integration including Graph API, OAuth, and webhook handling"
---
...
<CardGroup cols={2}>
  <Card title="MetaGraphApiService" icon="chart-network">
...
```typescript
app.meta.graphApiBaseUrl: string;        // Base URL for Graph API (defaults to https://graph.facebook.com)
app.meta.whatsappApiVersion: string;     // Default API version for WhatsApp operations (default v25.0 as of 2026-06-10)
app.meta.instagramApiVersion: string;    // Default API version for Instagram operations (default v25.0)
app.meta.messengerApiVersion: string;    // Default API version for Messenger operations (default v25.0)
app.meta.oauthApiVersion: string;        // API version for OAuth flows (default v25.0)
```
...
```typescript
app.meta.graphApiBaseUrl: string;    // Base URL for OAuth endpoints
app.meta.appId: string;              // Meta application ID
app.meta.appSecret: string;          // Meta application secret
app.meta.oauthApiVersion: string;    // API version for OAuth dialog and token exchange URLs (default v25.0)
```
...
```typescript
app.meta.webhookVerifyToken: string;  // Secret token for webhook verification
```
...
<Steps>
  <Step title="Error detection">
    Messaging classifies error code `131042` and related payment-method/billing/credit-line text as `MessageFailureCategory.META_BILLING`
  </Step>
  <Step title="Error preservation">
    The raw `failedReason` is preserved for debugging and audit purposes
  </Step>
  <Step title="Error exposure">
    The category is included on message DTOs and `message-status-updated` socket events
  </Step>
  <Step title="User guidance">
    The frontend can guide the user to Meta Business Suite billing settings based on the category
  </Step>
</Steps>
...
<Steps>
  <Step title="Update environment variable">
    Update the env var for the desired channel version
  </Step>
  <Step title="Deploy changes">
    Deploy the configuration change to your environment
  </Step>
  <Step title="Test functionality">
    Test the integration to verify compatibility with the new API version
  </Step>
</Steps>

<Tip>
No code changes needed when upgrading API versions—only configuration updates.
</Tip>
...
<Info>
The `externalEventId` field uses a unique constraint to prevent duplicate webhook processing, ensuring idempotent behavior even if Meta sends the same event multiple times.
</Info>