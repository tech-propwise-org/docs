...
description: "Complete API documentation for Property Finder's backend services including authentication, authorization, and endpoint specifications"
---
...
Call the Issue JWT Token Endpoint with your API key and API secret in the request body.
</Step>

<Step title="Use the access token">
Include the returned Bearer token in the `Authorization` header of your API requests.
</Step>
</Steps>
...
<Info>
Your access token will be valid for the duration (in seconds) indicated by the `expiresIn` field. Currently, tokens expire in **30 minutes**. No refresh token flow is supported.
</Info>
...
If you exceed your allotted rate limit, you will receive a `429 Too Many Requests` HTTP response:
...
<CardGroup cols={2}>
<Card title="Retry with backoff" icon="clock">
