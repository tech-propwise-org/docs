...
description: "Detailed technical implementation specification for the off-plan directory feature including frontend components, API integration, and UI patterns"
---
...
The screenshots provided show the target design from a competitor platform. Key visual patterns to replicate:

<CardGroup cols={2}>
  <Card title="List Page (Grid View)" icon="grid-2">
...
Replace the entire Real Estate section with a single Off-Plan entry.
</Step>

<Step title="Create Route Structure">
Set up the off-plan pages and routing system.
</Step>

<Step title="Build Component Structure">
Create all necessary components for list and detail views.
</Step>

<Step title="Implement API Layer">
Create the off-plan API service with proper TypeScript types.
</Step>
...
<AccordionGroup>
<Accordion title="Core Building DTO">
```typescript
...
<Accordion title="Unit and Group DTOs">
```typescript
...
offPlan: {
  ...
  buildingsInfinite: (filters: OffPlanBuildingFilters) => 
    [...queryKeys.offPlan.buildings(), 'infinite', filters] as const,
  buildingDetail: (id: number) => 
    [...queryKeys.offPlan.buildings(), 'detail', id] as const,
  buildingUnits: (buildingId: number) => 
    [...queryKeys.offPlan.buildings(), buildingId, 'units'] as const,
...
Add the `maxPreHandoverPercent` query parameter to the buildings search endpoint
</Step>

<Step title="Component Development">
Build out all the UI components following the design specifications
</Step>

<Step title="Integration Testing">
Test the complete flow from listing to detail views
</Step>

<Step title="Performance Optimization">
Implement proper caching and lazy loading for images and data
</Step>
</Steps>

<Warning>
Remember to remove all existing real estate routes and components that are being replaced by the off-plan directory.
</Warning>